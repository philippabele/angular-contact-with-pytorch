...
def load_data(train_data_path, val_data_path):
    ...
    train_features, train_targets = train_data[['Fr', 'Lifetime']], train_data['n']
    val_features, val_targets = val_data[['Fr', 'Lifetime']], val_data['n']
...
def train_model(train_dataloader, val_dataloader, model, loss_fn, optimizer, epochs, device):
    ...
    for epoch in range(epochs):
        for inputs, targets in train_dataloader:
            inputs, targets = inputs.to(device), targets.to(device).long() # Ensuring targets are long integers
            # Forward pass
